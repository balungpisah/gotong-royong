<script lang="ts">
	...
	const genesisPct = $derived(
		genesis.threshold > 0
			? Math.min(Math.round((genesis.meaningful_interactions_this_month / genesis.threshold) * 100), 100)
			: 0
	);
...
>
	<div class="rounded-xl border border-border/30 bg-muted/10 p-4">
		...
		<div class="mt-3 flex items-center gap-3">
			...
			{#if multiplierPct > 0}
				<span class="text-caption text-muted-foreground">{m.profil_consistency_bonus({ pct: String(multiplierPct) })}</span>
			{:else}
			...
			<div class="rounded-lg bg-muted/20 p-2.5">
				...
				<div class="mt-1 flex items-center gap-2">
					<div class="h-1.5 flex-1 rounded-full bg-muted/30">
						<div class="h-full rounded-full bg-primary transition-all duration-500" style="width: {qualityPct}%"></div>
					</div>
		...
		<div class="mt-4 rounded-lg border border-border/20 bg-muted/5 p-3">
			...
			<div class="mt-2">
				...
				{:else}
					<p class="mt-1 text-caption text-muted-foreground">{m.profil_genesis_remaining({ count: String(genesis.threshold - genesis.meaningful_interactions_this_month) })}</p>
				{/if}
			</div>
		</div>
	</div>
