<script lang="ts">
	...
	let {
		...
		onvote,
		bare = false
	}: { block: VoteBlock; onvote?: (optionId: string) => void; bare?: boolean } = $props();
...
<div
	class={cn('flex flex-col gap-3', !bare && 'rounded-lg border border-border bg-card p-4')}
	data-slot="vote-block"
>
	<div class="flex items-start justify-between gap-2">
		<p class="text-body font-bold text-foreground">{block.question}</p>
		<Badge variant="info" class="shrink-0 text-small"
			>{voteTypeLabels[block.vote_type] || block.vote_type}</Badge
		>
	</div>
	...
	<div class="flex flex-col gap-2">
		{#each block.options as option (option.id)}
			{@const optPercent =
				block.total_voted > 0 ? Math.round((option.count / block.total_voted) * 100) : 0}
			<Button
				variant="ghost"
				class={cn(
