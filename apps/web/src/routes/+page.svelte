<script lang="ts">
	...
	const selectedFeedItem = $derived.by(() => {
		...
		const match = feedStore.filteredStream
			.filter(
				(s): s is import('$lib/types').FeedStreamItem & { kind: 'witness' } => s.kind === 'witness'
			)
	...
	type SeedVisibilityFilter = 'all' | 'seeded' | 'non_seeded';
	...
	let seedVisibilityFilter = $state<SeedVisibilityFilter>('all');

	const seedCoverage = $derived.by(() => {
		let totalWitnessCards = 0;
		let seededWitnessCards = 0;
		for (const streamItem of feedStore.filteredStream) {
			if (streamItem.kind !== 'witness') continue;
			totalWitnessCards += 1;
			if (streamItem.data.dev_meta?.is_seed) seededWitnessCards += 1;
		}
		return {
			totalWitnessCards,
			seededWitnessCards,
			nonSeededWitnessCards: Math.max(0, totalWitnessCards - seededWitnessCards)
		};
	});

	const seedFilteredStream = $derived.by(() => {
		if (!dev || seedVisibilityFilter === 'all') {
			return feedStore.filteredStream;
		}
		return feedStore.filteredStream.filter((streamItem) => {
			if (streamItem.kind !== 'witness') return false;
			const isSeeded = streamItem.data.dev_meta?.is_seed === true;
			return seedVisibilityFilter === 'seeded' ? isSeeded : !isSeeded;
		});
	});
	...
	const masonryStream = $derived.by<MasonryItem[]>(() => {
		if (feedStore.filter !== 'semua') {
			return seedFilteredStream;
		}
		return [triageEntry, ...seedFilteredStream];
	});
		...
		>
			...
			{:else if selectedWitnessId && witnessStore.detailError}
				<div class="flex h-full items-center justify-center p-6">
					<div
						class="w-full max-w-sm rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-center"
					>
						...
						<div class="mt-3 flex justify-center gap-2">
							<Button variant="outline" size="sm" onclick={retryWitnessDetailLoad}>
								Coba lagi
							</Button>
							<Button variant="ghost" size="sm" onclick={closeDetail}>Tutup</Button>
						</div>
					</div>
				</div>
			{/if}
...
<div class="mx-auto w-full px-4 lg:px-4">
	...
	<div class="flex w-full items-start gap-4">
		<!-- ── LEFT: Feed column ──────────────────────────────────────── -->
		<div class="min-w-0 flex-1 flex flex-col" role="region" aria-label="Activity feed">
			<!-- Feed content -->
			<section class="flex flex-1 flex-col gap-3">
				{#if dev}
					<div
						class="flex flex-wrap items-center gap-2 rounded-xl border border-border/40 bg-muted/20 px-3 py-2"
					>
						<span class="text-small font-medium text-muted-foreground">Seed filter</span>
						<Button
							variant={seedVisibilityFilter === 'all' ? 'secondary' : 'outline'}
							size="sm"
							onclick={() => {
								seedVisibilityFilter = 'all';
							}}
						>
							Semua ({seedCoverage.totalWitnessCards})
						</Button>
						<Button
							variant={seedVisibilityFilter === 'seeded' ? 'secondary' : 'outline'}
							size="sm"
							onclick={() => {
								seedVisibilityFilter = 'seeded';
							}}
						>
							Seeded ({seedCoverage.seededWitnessCards})
						</Button>
						<Button
							variant={seedVisibilityFilter === 'non_seeded' ? 'secondary' : 'outline'}
							size="sm"
							onclick={() => {
								seedVisibilityFilter = 'non_seeded';
							}}
						>
							Non-seeded ({seedCoverage.nonSeededWitnessCards})
						</Button>
					</div>
				{/if}
				...
				{#if feedStore.error && feedStore.filteredStream.length > 0}
					...
					>
						...
						<Button variant="outline" size="sm" onclick={retryFeedLoad}>Coba lagi</Button>
					</div>
				{/if}
				...
				{#if witnessStore.listError}
					...
					>
						...
						<Button variant="outline" size="sm" onclick={retryWitnessListLoad}>Coba lagi</Button>
					</div>
				{/if}
				...
				{#if selectedWitnessId && witnessStore.detailError}
					...
					>
						...
						<Button variant="outline" size="sm" onclick={retryWitnessDetailLoad}>Coba lagi</Button>
					</div>
				{/if}
