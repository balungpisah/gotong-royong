...
| Action Type | Maps To |
|---|---|
| `schema:InformAction` | → Catatan Komunitas |
| `schema:RepairAction` | → Komunitas |
| `schema:CreateAction` | → Komunitas |
| `schema:SearchAction` | → Komunitas |
| `schema:AchieveAction` | → Komunitas |
| `schema:AssessAction` | → Komunitas |
| `schema:AlertAction` | → Siaga |

Catatan Saksi routing is user-initiated ("Saya ingin mencatat untuk diri sendiri") — not derived from Action type.

AI-00 produces the triples including the Action type. Mode routing reads the Action type triple to determine destination. Track hint labels are **dynamically generated by AI** — not fixed to the old 5-label set. They are UI display labels only.
...
| Separate track hint classification | Track hint labels dynamically generated by AI. Display-only, not fixed to predefined labels. |
...
| ADAPTIVE-PATH-SPEC-v0.1.md | `track_hint` dynamically generated by AI (not fixed). `seed_hint` remains. |
