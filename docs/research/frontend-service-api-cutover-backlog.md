...
| `FEED-SEED-003` | Seeded taxonomy matrix visibility | We need explicit seeded-card labeling/filtering during taxonomy/design iteration without polluting production semantics. | Implemented on 2026-02-27: kept optional `FeedItem.dev_meta` / `payload.dev_meta` contract mapping (`is_seed`, `seed_batch_id`, `seed_origin`) and moved practical taxonomy seed source to backend dev seeding (`scripts/dev/seed-api.sh` + `scripts/dev/seed-feed-matrix.json`, `seed_origin=db`) so `/v1/feed` is the source of seeded matrix cards; dev-only feed affordances (`SEED` badge + `all/seeded/non-seeded` toggle) remain unchanged on home feed rendering. Canonical reference: `docs/research/contracts/feed-seed-metadata-v1.md`. |
...
## 7) AI Endpoint Registry Guardrail (2026-02-26)

- Any new AI/LLM endpoint slice must register endpoint details in `docs/research/contracts/ai-endpoint-map-v1.md` before implementation merge (prompt/version, input/output contracts, fallback, validation gate, observability, and code references).
