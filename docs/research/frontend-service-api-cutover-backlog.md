...
| `FEED-SEED-003` | Seeded taxonomy matrix visibility | We need explicit seeded-card labeling/filtering during taxonomy/design iteration without polluting production semantics. | Implemented (initial) on 2026-02-26: added optional `FeedItem.dev_meta` / `payload.dev_meta` contract mapping (`is_seed`, `seed_batch_id`, `seed_origin`), marked fixture taxonomy cards as seeded (`seed_origin=fixture`), and added dev-only feed affordances (`SEED` badge + `all/seeded/non-seeded` toggle) on home feed rendering. Canonical reference: `docs/research/contracts/feed-seed-metadata-v1.md`. |
