...
usage() {
  cat <<'EOF'
usage: scripts/deploy/pack_c_stage_kickoff.sh --stage <stage-a|stage-b|stage-c> [--namespace <k8s-namespace>] [--output <report-path>] [--observation-hours <hours>] [--run-readiness <true|false>]
EOF
}
...
if [[ "${READINESS_RESULT}" == "fail" || "${STAGE_RESULT}" == "apply_failed" || "${STAGE_RESULT}" == "dry_run_failed" ]]; then
  exit 1
fi
